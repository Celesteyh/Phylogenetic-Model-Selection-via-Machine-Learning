...
class ResNet(nn.Module):
    def __init__(
        ...
        self.attention = nn.MultiheadAttention(embed_dim=768, num_heads=8, batch_first=True)  # B * 169 * 768
        self.relu = nn.ReLU(inplace=True)
        self.fc1 = nn.Linear(768, 72)
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        # flatten_parameters(). (Triggered internally at  ../aten/src/ATen/native/cudnn/RNN.cpp:925.)
        ...
        x, _ = self.attention(x, x, x)  # B * 169 * 768
        x = torch.mean(x, dim=1)  # B * 768
        x = self.fc1(x)
        x = self.relu(x)
        x = self.fc2(x)
