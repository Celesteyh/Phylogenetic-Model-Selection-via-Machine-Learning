...
# if float32 is used here instead of float16, the sum of aa_freqs will not be 1
def make_profile(m, num_sumstats=625):
    n_taxa = m.shape[0]
    n_sites = m.shape[1]
    # mm is the extracted features of the MSA, which contains num_sumstats pairs and 440 features for each pair
    # invariant # 20, replacements between groups # 380, base frequencies # 40
...
def sitewise_aa_freq(m):
    ...
    for i in range(20):  # note the range here
        mm[:, i] = np.sum(m == i * 2 + 1, axis=0) / n_taxa  # (1, n_sites) normalizing by n_taxa so that each row sums to 1
...
def extract(file):
    try:
        with open(path1 + file, 'rb') as f:
            ...
            RHAS_feature = make_RHAS(msa)
            # save the feature to a file in path2
            np.save(path2 + file[:-4] + '.npy', RHAS_feature)
...
path1 = 'F:\\simulation\\'
path2 = 'E:\\RHAS_feature\\'

if __name__ == '__main__':
    start_time = time.time()
    # path1 = '/scratch/dx61/yd7308/b_mimicked_vali_MSA/'
    # path1 = 'F:\\simulation\\'
    # path2 = '/scratch/dx61/yd7308/b_mimicked_vali_feature/'
    # path2 = 'E:\\RHAS_test\\'
    files = os.listdir(path1)[:20000]
    # for file in files:
    #     try:
    #         with open(path1 + file, 'rb') as f:
    #             lines = f.readlines()
    ...
    #         # feature = make_profile(msa)
    #         RHAS_feature = make_RHAS(msa)
    #         print(RHAS_feature.shape)
    #         print(RHAS_feature[:5])
    #         # print('----------------------------')
