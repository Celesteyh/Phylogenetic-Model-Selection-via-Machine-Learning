...
replacements = np.array([2, 24, 160, 896, 4608, 22528, 106496, 491520, 2228224, 9961472, 44040192, 192937984, 838860800,
                         ...
                         5234491392, 20937965568, 83751862272, 335007449088, 1340029796352, 5360119185408], dtype=np.uint64)
...
def calculate_base_freq(msa, gaps):
    ...
    base_freqs = np.zeros((n_taxa, 20), dtype=np.float32)  # (n_taxa, n_aa) increase the precision here!!!!
    ...
    for i_taxa in range(n_taxa):
        ...
        # print(counts)
        base_freqs[i_taxa] = counts / np.sum(counts)  # changed n_sites to np.sum(counts)
...
def replace_gaps(site):
    ...
    temp = site[site != 0]
    if len(temp) == 0:
        site[site == 0] = np.random.choice(20) * 2 + 1  # randomly choose an amino acid
    else:
        counts = np.bincount(temp, minlength=40)
        max_count = counts.max()
        # find all amino acids that have the maximum frequency
        most_common_amino_acids = np.where(counts == max_count)[0]
        # randomly select one if there is a tie
        most_common = np.random.choice(most_common_amino_acids)
        site[site == 0] = most_common
    return site
...
def make_profile(m, num_sumstats=1225):
    ...
    m = np.apply_along_axis(replace_gaps, 1, m)
    # m = np.vectorize(amino_acid_map.get)(m)
    # get the base frequencies of each sequence
    base_freqs = calculate_base_freq(m, gaps=False)
    ...
    unique_replacement_freqs = np.zeros((unique_pairs.shape[0], 400), dtype=np.float32)
    for i_pair, pair in enumerate(unique_pairs):
        replacement_count = np.zeros(400, dtype=np.float32)
        ...
        for i, r in enumerate(replacements):
            ...
            replacement_count[i] = count / n_sites
...
if __name__ == '__main__':
    ...
    path2 = '/scratch/dx61/yd7308/real_feature_replace/'
    ...
    # real1 = "E:/PycharmProjects/Phylogenetics/aln/real/6PF01142+F+R4.phy"
